...
window.$agRendererTemplate = {
	// 적용 버턴 클릭 핸들러
	myApplyBtnClick: (rowIndex: number, event: Event): void => {
	...
	//팝업 버턴 클릭 핸들러
	myPopupBtnClick: (rowIndex: number, event: Event): void => {
		alert(`eventType : ${event.type}, rowIndex :  ${rowIndex}  팝업 버턴 클릭`);
	},

	// 셀렉트 변경 핸들러
	mySelectChangeHandler: (rowIndex: number, selectedValue: string, event: Event) => {
		const myGridID = '#aui-grid-wrap-RendererTemplate';
		// 그리드에 실제 업데이트 적용 시킴
		$ag.updateRow(
			myGridID,
	...
	// 체크박스 그룹 클릭 핸들러
	myCheckClick: (rowIndex: number, event: Event): void => {
