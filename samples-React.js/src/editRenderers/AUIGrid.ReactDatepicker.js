...
import { agUtils } from '../static/AUIGrid-React.js/AUIGridReact.js';
// React Date Picker is licensed under the MIT License.
...
export default window.AUIGrid.Class({
	...
	/* CSS 스타일 클래스 */
	cssClass: 'aui-grid-edit-renderer-custom aui-grid-react-date-picker',
	...
	create: function () {
		...
		// 현재 값
		const value = new Date(this.data[this.dataField]);

		// datepicker 에 전달할 속성 정의 값들
		const extraProps = this.extraProps;

		// datepicker 에서 inline = true 강제 시킴
		extraProps.inline = true;

		// 리액트 DatePicker 컴포넌트 렌더링
		this.__reactDomNode.render(
			React.createElement(DatePicker, {
				selected: value,
				onChange: this.__updateDate.bind(this),
				...extraProps
			})
		);
	},
